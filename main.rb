...
state_manager.publish_event(MaxEvent.new(name: MaxEvent::Events::HUNGER_ATTACKED))
puts "Max just got a hunger attack! What would his state be now?"
puts "Max's state is now: #{state_manager.machine.state}"
state_manager.publish_event(MaxEvent.new(name: MaxEvent::Events::ARRIVED_TO_GYM, metadata: { gym_name: 'Target Ramat Gan' }))
...
state_manager.publish_event(MaxEvent.new(name: MaxEvent::Events::ARRIVED_TO_GYM))
state_manager.publish_event(MaxEvent.new(name: MaxEvent::Events::ARRIVED_TO_GYM))

begin
  state_manager.publish_event(MaxEvent.new(name: 'invalid_event'))
rescue => e
